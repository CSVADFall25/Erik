...
function preload() {
  ...
  table = loadTable('tweet_counts.csv', 'csv', 'header');
}
...
function setup() {
  ...
  for (let r = 0; r < 100; r++) { 
      if(earliestDate === undefined || compareDates(formatDate(table.getString(r, 'date')), earliestDate) === -1) {
          earliestDate = formatDate(table.getString(r, 'date'));
    }
    if(latestDate === undefined || compareDates(formatDate(table.getString(r, 'date')), latestDate) === 1) {
      latestDate = formatDate(table.getString(r, 'date'));
    }
    labels.push(formatDate(table.getString(r, 'date')));
    ...
    // distance.push(float(random(5)));
    num_tweets.push(float(table.getString(r, 'num_tweets')));
    // duration.push(float("10.0"));
  }
...
function draw() {
  drawBarChart(num_tweets, num_tweets, labels);
  drawAxes();
...
function drawBarChart(values1, values2, labels) {
  background(240);
